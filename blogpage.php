<?php 
include('xp-admin/include/db.php');
...
$selectPost=$conn->prepare("SELECT * FROM post WHERE slug = '$post_slug' OR slug = '$post_slug_slash' AND status='Publish'");
$selectPost->execute(); 
...
if($countpost>0){
        while($row=$selectPost->fetch(PDO::FETCH_ASSOC)){ 
           ...
           if(!empty($row['products'])){
            ...
            $tableOfContent = $row['tableOfContent'];
            $table_array=explode(",",$tableOfContent);
